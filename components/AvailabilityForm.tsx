...
type Availability = { start: Date; end: Date };
...
const INSTRUMENTS = [
  ...
  'Saxophone', 'Violin', 'Cello', 'Trumpet', 'Other',
];
...
export default function AvailabilityForm() {
  ...
  function handleInstrument(i: string) {
    setForm(f => ({
      ...f,
      instrument: f.instrument.includes(i)
        ? f.instrument.filter(x => x !== i)
        : [...f.instrument, i],
    }));
  }
  ...
  function handleSlotChange(e: React.ChangeEvent<HTMLInputElement>) {
    setForm(f => ({
      ...f,
      slot: { ...f.slot, [e.target.name]: e.target.value },
    }));
  }
  ...
  function addSlot() {
    ...
    if (form.availabilities.some(a => isOverlap(a, newSlot)))
      return setError('Slot overlaps with existing');

    setForm(f => ({
      ...f,
      availabilities: [...f.availabilities, newSlot],
      slot: { start: '', end: '' },
    }));
    setError('');
  }
  ...
  return (
    <form
      id="signup"
      className="max-w-lg mx-auto bg-white dark:bg-gray-800 rounded shadow p-6 mt-8"
        ...
        <div className="flex flex-wrap gap-2">
          {INSTRUMENTS.map(i => (
            <button
              ...
              className={`px-3 py-1 rounded border ${form.instrument.includes(i)
